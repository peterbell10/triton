...
@pytest.mark.parametrize("num_ctas", num_ctas_list)
def test_scan_slice(op, dtype_str, XBLOCK, RBLOCK, num_ctas, device):
    if is_hip():
        pytest.skip("test_scan2_slice is not supported in HIP")
    ...
    def kernel(out_ptr, in_ptr, XBLOCK: tl.constexpr, RBLOCK: tl.constexpr):
        ...
        GENERATE_TEST_HERE
        tl.store(out_ptr + xindex * RBLOCK + rindex, result[None, :])

    patch = f'result = tl.{op}(data, axis=0)'
    kernel = patch_kernel(kernel, {'GENERATE_TEST_HERE': patch})
