...
mlir::triton::translateModuleToLLVMIR(Operation *module, llvm::LLVMContext &llvmContext, llvm::StringRef name) {
  auto llvmMod = mlir::translateModuleToLLVMIR(module, llvmContext, name);

  // Workaround for MLIR not exposing a way to add NoReturn to a function declaration
